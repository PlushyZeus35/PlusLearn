...
function displayQuestion(questionId){
    ...
    if(targetQuestion.length==1){
        ...
        let correctInput = getInput('correctInput','Respuesta correcta', targetQuestion[0].correctAnswer);
        ...
        let incorrectInput = getInput('incorrect1Input','Respuesta incorrecta', inc[0]);
        incorrectInput.classList.add('incorrectInput');
        mainScreen.appendChild(incorrectInput);

        let incorrectInput2 = getInput('incorrect2Input','Respuesta incorrectas', inc[1]);
        incorrectInput2.classList.add('incorrectInput');
        mainScreen.appendChild(incorrectInput2);

        let incorrectInput3 = getInput('incorrect3Input','Respuesta incorrectass', inc[2]);
...
function questionUpdated(){
    ...
    for(let i=0; i<questions.length; i++){
        if(questions[i].id == questionTargeted){
            ...
            questions[i].isUpdated = questions[i].isNew ?  false : truea;
        }
    }
}
